...
<head>
    <meta charset="UTF-8">
    <title>DateTime Practice</title>
<head>
<?php # Script 16.5 - DateTimePractice.php

// Set the start and end date as today and tommorow as default:
$start = new DateTime();
$end = new DateTime();
$end->modify('+1 day');
...
if (isset($_POST['start'], $_POST['end'])) {
    ...
    if ((list($sy, $sm, $sd) = validate_date($_POST['start'])) && (list($ey, $em, $ed) = validate_date($_POST['end']))) {
        ...
        if ($start < $end) {
            ...
            // Print the results:
            echo '<p>The event has been planned starting on ' . $start->format($format) . " and ending on " .
            $end->format($format) . ", which is a period of " . $interval->days . " day(s).</p>";
...
<h2>Set the Start and End Dates for the Event</h2>
<form action="DateTimePractice.php" method="post">
    <p><label for="start">Start Date: </label>
    <input type="date" name="start" value="<?php echo $start->format($format); ?>"> (YYYY-MM-DD)</p>
    <p><label for="end">End Date: </label>
    <input type="date" name="end" value="<?php echo $end->format($format); ?>"> (YYYY-MM-DD)</p>
